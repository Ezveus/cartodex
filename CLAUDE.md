...
be a second store, not a migration of this one), and roles anywhere but `/archetypes/:id`, the
admin screen and `/cards`'s filter bar (#164) — not on a deck page, not in the JSON API, not in an
MCP tool.
