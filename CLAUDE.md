...
**The Ruby version is declared six times over, and `.ruby-version` is the one that decides.** All six jobs in `.github/workflows/ci.yml` say `ruby-version: .ruby-version`, so that file is what the five checks and the deploy actually run under; `mise.toml`, the production `Dockerfile`'s `ARG RUBY_VERSION`, the devcontainer's copy of it, `config/deploy.yml`'s commented Kamal build-arg example and this file's own overview line all follow it, and `test/ruby_version_test.rb` refuses any disagreement — the workflow included, because pinning one job to a literal would leave every other declaration agreeing about a version the gates no longer run. Three of the six are traps. **`mise.toml` wins over `.ruby-version`** where they differ and nothing says so: the repository pinned 3.4.1 for months while every local command ran 4.0.1, so `bin/rubocop` reported `analyzing as Ruby 3.4, running on ruby 4.0.1` and no local lint result could be compared with CI's — which is why the guard also asserts the *running* interpreter, a tautology in CI and in the image (both install from that file) and the whole point on a developer's machine. The Kamal build arg takes the **bare** version, never the `ruby-` prefixed spelling: it lands in `ruby:$RUBY_VERSION-slim`, and `ruby:ruby-4.0.6-slim` is not an image — Rails generated that line prefixed, so it was wrong from the first commit and waiting for whoever uncommented it. And a historical version named in this file has to be written lowercase or as the image tag it came from, since the guard requires every "Ruby x.y.z" here to be the current one. Moving the reference is therefore a six-file edit, and the deploy's base distro moves with it: 3.4.1 meant `ruby:3.4.1-slim` on Debian 12 bookworm, 4.0.6 means `ruby:4.0.6-slim` on Debian **13 trixie**, which raised the runtime libvips from 8.14.1 to 8.16.1. **Pin a recent patch release and keep pinning one**: nothing here bumps it — `.github/dependabot.yml` covers bundler and github-actions, neither Docker nor `.ruby-version` — so the pin only ever moves when somebody does it by hand, and 4.0.1 was already five releases and eight months stale, inside the affected range of CVE-2026-46727, on the day this file first named it. **No CI check builds or boots the image**, so a distro jump is first exercised on the builder during `kamal deploy` itself.

## Architecture

### Deep dives

Eight features have their rationale written out under `docs/architecture/`, one file each. What
...
| `deck-odds.md` | `/decks/:id/odds` — the deal model and its enumeration proof, card groups, the precomputed curves, the combination calculator, the five refusals, the public surface |
...
**A set code means one thing, and `Card.in_set_code` is where it means it.** Every surface that
...
**`CardSearchable` reads "POR 56" as a printing, and where that collides with a name the set
wins.** The parser behind `/cards`, `Api::CardsController` and the spotlight pops a trailing number
and then a set code; the code pop is guarded by `tokens.length > 1 || number`, and **both halves
are load-bearing and now coupled**. `|| number` is what lets a two-token query reach
code+number with an empty name — without it "POR 56" searched for a card *named* "POR". And
...
**`search_cards` takes `query`, `set_code` and `set_number` and requires none of them** — set plus
number is UNIQUE, so a caller holding a printing reference does not know the name. Three details
bite. The criterion-less refusal tests **`blank?`, never `nil?`**: `Card.name_matching("")`
compiles to `LIKE '%%'`, so a `nil?` guard answers `{"query": ""}` — legal under the schema — with
20 arbitrary rows out of 4732. `set_number` is declared `type: [ "string", "integer" ]` because
`mcp`'s `validate_tool_call_arguments` defaults to **true** and enforces the declared schema on the
wire, while an assistant reading "86" off a card sends the integer as often as the string; it is
then matched **exactly, as text** (100 printings carry `GG1`-style numbers, none is zero-padded)
after `squish` and not `strip`, since a copy-paste out of a web page carries U+00A0 that
`String#strip` leaves in place. And the refusal goes through `McpTool.error_text`, not `text`:
`isError` is the only field a client can read without parsing English, and the gem's own
"Missing required arguments" — which this refusal replaces — sets it. The nine older
`text("Error: …")` refusals in `app/mcp/` still ship `isError: false`; that is a pre-existing gap,
not this tool's convention.
...
`bin/rails icons:build` rasterises the three icon SVGs into the committed PNGs and needs `:environment` — not for models but because it reads an autoloaded constant to lift that same `svgload` block. Rasterising on a different libvips/librsvg re-encodes those PNGs **byte-differently while the pixels are identical** (measured in the trixie image: every hash moves, `max_abs_diff 0.0`), so `git status` naming all six after a no-op run means the toolchain moved and not the icons — compare the pixels before committing the churn. `bin/rails og:default_banner` rewrites `public/og-default.jpg`.

**The mark now lives in a fourth place, and a test is what binds it to the other three.** `Ui::Logo` redraws `public/icon-small.svg`'s three shapes inline in the navbar brand, because the mat's fill in that file is `#0E1320` — exactly `--ink-900`, exactly `.navbar`'s background — so an `image_tag` of it puts an invisible body on the bar, and an external SVG's fills are out of the page's reach. Inline, the three fills become tokens (`--ink-700` / `--ink-500` / `--flare`). `icons:build` cannot cover that derivative, so `test/components/ui/logo_test.rb` parses both files and fails on any geometry divergence — and asserts the fill mapping **literally** rather than "differs from the source", since `var(--ink-900)` differs from `#0E1320` as a string and *is* it as a colour, which is the single failure the component exists to prevent.

**How a deck *opens*, as against how it has *done*** — `/decks/:id/odds`, `Decks::Odds::Deal` / `Groups` / `Report` / `Combo`, the twelve Phlex components and the two Stimulus controllers — is written out in **`docs/architecture/deck-odds.md`**: the permutation model, the enumeration proof, the fingerprint grouping, the precomputed curves, the combination calculator and the five states the page refuses to compute. Read it before touching anything under `app/services/decks/odds/`. Four of its rules bite from outside their own feature:

**`Decks::Odds::Deal` must never reference `Deck`, `Card` or Active Record**, and that is what buys the feature its verification rather than being tidiness: taking plain integers, it is checked against exhaustive enumeration of all 40320 permutations of an 8-card deck in exact `Rational` arithmetic — equalities, not agreements to a tolerance — which is impossible for a service that needs fixtures. It is also why `Deal` takes `hand_size:`/`prize_count:` at all: a 60-card deck is 60! permutations, and an 8-card deck dealt a 7-card hand leaves no prize block, while the *shape* of the question is identical at both sizes. The page's own arithmetic lives there too and nowhere else: `Report` precomputes a whole curve per group and the Stimulus controllers only index into it, because this repository has **no JS test infrastructure** and a second implementation of the conditional hypergeometric would be held down by nothing at all.

**A Basic Pokémon is `card_type == "Pokémon" AND stage == "Basic"`, never `stage` alone.** Measured on the development catalogue: 2196 Pokémon carry `stage = "Basic"` — and so do **50 Basic Energy cards**. Any future reader of `stage` has the same trap waiting, and here it made the mulligan rate wrong in the reassuring direction on exactly the decks that play the most Energy.

**`Decks::Odds::Groups` preloads `deck_cards: :card` itself**, so `DecksController#odds` deliberately does **not** — measured at two extra statements, 8 against 6, and both served by the query cache, which means `SQLCounter` cannot see them and no flat-cost test in this repository would ever report the waste. That cache has fooled a measurement on this feature three times; a query count that could be served from it belongs inside `ActiveRecord::Base.uncached`, which is what `Groups`' own flat-cost test and the combination frame's do.

**`odds` reuses `DeckPolicy#show?` and grows no rule of its own**, because the page is a pure function of the decklist that `#show` already renders in full and a second rule could only ever come to disagree with itself. It is the app's third unscoped deck lookup, after `#show` and `#export`, with `authorize` as the next line; `#stats` stays owner-only. Its rate limit carries its own `name: "decks-odds"` — sharing `"decks-export"` merges the two budgets — and its `unless: -> { user_signed_in? }` is what stops the deck's own owner being throttled. It is also the app's **tenth** `rate_limit` and the first on an action a Turbo Frame navigates to, which is a trap the other nine do not have: the refusal has no body (there is no `public/429.html`), so Turbo's frame loader replaces nothing and does not even dispatch `turbo:frame-missing` — the click leaves no trace on screen. The combination therefore watches `turbo:before-fetch-response` for a 429 and unhides a notice shipped with the page, and **any future `rate_limit` on a frame-navigated action inherits the same silence**. See `docs/architecture/public-surface.md`.

**The design system, the global search spotlight, the navbars and the shared form components** are written out in **`docs/architecture/frontend.md`** — the CSS custom-property token system at the top of `application.css` and its three override layers, `/styleguide`, `Search::Spotlight` and the four details that keep its trigger and its field from working against each other, `Ui::NavbarShell`, `Ui::NavLinks.section_for` and `Ui::NavGroup` (with the two measurements that forbid `white-space: nowrap` on `.navbar-link` and set `.navbar-inner`'s gap at 1.5rem), `Ui::CardSelect`, and `Ui::StandardPoolNotice`. Read it before adding a component, a navbar entry or a design token — **a navbar entry in particular**: the row does not shrink to fit, and nine of them were 661px off the document at 769px before they were grouped.
...
- **Navigation differs.** Below the breakpoint `.navbar-menu` is `display: none` until the hamburger toggles `.navbar-menu--open` onto it, and Capybara will not click what it cannot see. Never click a nav link directly — use `click_nav_link`, which opens the menu below the breakpoint (and retries, because the toggle click and the link click straddle a Turbo page swap) and, above it, opens the `Ui::NavGroup` panel containing the link. It still takes the **leaf's** label and nothing else: `"My decks"`, never `"Decks"`. Its group lookup uses the space-padded `normalize-space(@class)` XPath idiom rather than `contains(@class, 'navbar-group')`, because the latter also matches `navbar-group-panel` — the link's own nearest ancestor — and then silently opens nothing.
