...
**An archetype's *address* is a slug of its name, and the two archetype pages are public.** `archetypes.slug` is `name_normalized` parameterized — stored, `NOT NULL`, UNIQUE, recomputed by a `before_validation` on every save, so renaming an archetype moves `/archetypes/:id` and **nothing records the old address**: links to it break, deliberately. `Archetype#to_param` returns `slug_in_database || slug` — the value the database holds, not the one in memory — which is what stops a *refused* admin rename from re-rendering a form that posts to the archetype whose slug the rejected name collided with, and renames the wrong row with a 200. Both lookups key on the column (`ArchetypesController#show`, `Admin::ArchetypesController#set_archetype`, the latter for the reason `Admin::DecksController` keys on `decks.key`); `Api::ArchetypesController` keeps integer ids, because its JSON `id` feeds a `<select>` and a `deck.archetype_id` write — a reference to a row, not the address of a page, the same split `decks.id` keeps against `decks.key`. `before_validation :assign_slug` is declared **after** `auto_generate_name`, and the order is load-bearing: that callback supplies the name when nobody typed one, so a slug computed first would be blank, blank is refused, and every `Api::ArchetypesController` create would 422. Two refusals, both on `:name` because that is the only field the admin form has — a collision (a UNIQUE slug adds the rule "names must differ by more than punctuation"; measured, two of the catalogue's 1806 card names collide, both Nidoran gender-symbol pairs) and a blank (`parameterize` returns `""` only for a name with no Latin-transliterable character, which is what #111 will produce). Fixtures spell `slug` out by hand beside `name_normalized`, and `ArchetypeTest` asserts every one of them against `name.squish.downcase.parameterize` — plus that the *migration's* own copy of that rule agrees with the callback, since `db:test:prepare` loads the schema and CI never runs a migration. See `docs/architecture/public-surface.md` and `docs/architecture/archetype-metagame.md`.
