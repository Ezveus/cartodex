class DeckCard < ApplicationRecord
  ...
  # Columns qualified on purpose: `collections` carries a `quantity` too, so an unqualified
  # comparison would go ambiguous the day this is merged into a join with it.
  scope :with_proxies, -> { where("deck_cards.owned_copies < deck_cards.quantity") }
