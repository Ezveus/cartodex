...
- **Backfill:** `StandardPools::AnchorBackfill` is a service, so it is tested like one — tournaments by `at(date)`, decks on `current`, non-Standard rows left alone, an event older than the whole history falling back to the oldest pool, idempotency, and the empty-database case reporting rather than writing. Moving the backfill out of the migration is what makes this testable at all. The run against the development database is still done by hand and its counts reported, since only that exercises the real rows. That by-hand run against real data has not happened yet: the worktree the backfill was implemented in has an empty development database (seeded pools, no users, no decks, no tournaments), so `standard_pools:backfill_anchors` there only confirmed `0/0` — a vacuous result, not a verification. Running it against real rows, after `db:seed`, in the main checkout, is the maintainer's outstanding step.
