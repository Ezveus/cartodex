...
**The form pre-fills everything that does not change.** A set release moves only the upper bound, so `first_card_set_id` and `regulation_marks` come from `StandardPool.current`. What is left to type is the new set, its release date and its legality date — exactly the part a human knows and the schema does not. *(Deviation, recorded during implementation: an earlier draft of this section also proposed `legal_on` as `released_on + 14 days`. That was dropped and replaced by a hint on the field saying "about two weeks after release". The two-week rule is not a rule — this same spec's ASC counter-example is five weeks — and a pre-filled wrong date is accepted without thought, where a blank field with a hint is typed from the announcement. The seed already stores `legal_on` for exactly this reason; the form should not re-derive what the seed refuses to.)* The annual rotation is the one case where the lower bound and the marks are touched, and it is the one case worth typing in full.
