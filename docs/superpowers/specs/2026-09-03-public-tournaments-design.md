...
**System tests.** The only tournament coverage in the browser today is the two tests in `standard_pools_test.rb` that exercise the pool-follows-date Stimulus controller on the tournament form; they survive the split untouched, since the event form keeps both fields they read. Two tests are added, both expected to pass on each side of the 768px breakpoint and navigating with `click_nav_link`: in stage 1, the nominal path "search the catalog → it is not there → create it → record my participation"; in stage 2, `public_navigation_test.rb` gains a visitor reaching the catalog and an event page and seeing no member affordance.
